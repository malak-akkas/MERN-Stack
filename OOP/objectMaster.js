...
//1
const diviedbythree = pokémon.filter(p => p.id % 3 == 0);
//2
...
//5
const namesAndIds = pokémon.filter(p => p.id > 99).map(p => p.name);
//6
const namesAndTypes = pokémon.filter(p => p.types.includes("poison")).map(p => p.name);
//7 
const secondType = pokémon.filter(p => p.types[1] == "flying").map(p => p.types[0]);
//8
const countpokémon = pokémon.filter(p => p.types.includes("normal")).length;


console.log(bListPkmn);
console.log(pkmnIds);
console.log(diviedbythree);
